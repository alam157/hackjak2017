        ...
        <script>
            ...
            function drop() {
                ...
                for (var i = 0; i < coordPuskesmas.length; i++) {
                    addMarkerWithTimeout(coordPuskesmas[i], i * 100, i, puskesmas[i]);
                }
            }
...
//                    }));
                    if (data.type == 'puskesmas') {
                        ...
                        var contentString = '<div id="content">' +
                                ...
                                '<div id="bodyContent">' +
                                '<p>Alamat : ' + data.alamat + '<br>' +
                                'Telepon : ' + data.telp + '<br>' +
                                'Fax : ' + data.fax + '<br>' +
                                'Email : ' + data.email + '<br>' +
                                'Website : ' + data.website + '<br>' +
                                '</p>' +
                                '<a href="<?= base_url() ?>index.php/home/detail/' + data.id + '" class="btn btn-primary btn-sm float-right">Detail</a>' +
                                '</div>' +
                                '</div>';
                    } else {
                        markers[i] = new google.maps.Marker({
                            ...
                            map: map,
                            animation: google.maps.Animation.DROP,
                            icon: '<?= base_url() ?>assets/images/logo_rs.png'
                        });

                        var contentString = '<div id="content">' +
                                '<div id="siteNotice">' +
                                '</div>' +
                                '<img src="<?= base_url() ?>assets/images/logo_rumahsakit.png" class="logo">' +
                                '<h2 id="firstHeading" class="firstHeading">' + data.nama + '</h2>' +
                                '<div id="bodyContent">' +
                                '<p>Alamat : ' + data.alamat + '<br>' +
                                'Telepon : ' + data.telp + '<br>' +
                                'Fax : ' + data.fax + '<br>' +
                                'Email : ' + data.email + '<br>' +
                                'Website : <a href="http://' + data.website + '">' + data.website + '</a><br>' +
                                '</p>' +
                                '<a href="<?= base_url() ?>index.php/home/detail/' + data.id + '" class="btn btn-primary btn-sm float-right">Detail</a>' +
                                '</div>' +
                                '</div>';
